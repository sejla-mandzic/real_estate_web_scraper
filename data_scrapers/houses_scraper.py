...
def scrape_house(page, line: str):
    id = int(line)
    ...
    for tr in trs:
        tds = tr.find_all("td")
        if tds[1].get_text(strip=True) == "✓":  # bools
            ...
            elif tds[0].get_text(strip=True).lower() == "kvadrata":
                area = float(tds[1].get_text(strip=True))
...
def scrape_houses():
    with sync_playwright() as p:
        ...
        file = open(os.path.join(os.path.dirname(os.path.dirname(__file__)), "houses.txt"), "r")
        ...
        for i, line in enumerate(lines):
            try:
                house = scrape_house(page, line.strip())
                print(house)
                houses.append(house)
        ...
        with open(os.path.join(os.path.dirname(os.path.dirname(__file__)), "houses.csv"), "w") as csv_file:
            csv_file.write(
